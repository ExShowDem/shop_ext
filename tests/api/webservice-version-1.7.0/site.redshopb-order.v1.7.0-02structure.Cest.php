...
/**
 * Class SiteRedshopbOrder170structureCest
 * @since 2.8.0
 */
class SiteRedshopbOrder170structureCest
{
	...
	public function prepare(redshopb2b $I)
	{
		...
		$this->category['name'] = $this->faker->bothify('SiteRedshopbOrder170structureCest category ?##?');
		$this->category['id'] = (int)$I->createCategory($this->category['name']);

		$I->comment('I create a product');
		$this->product['name'] = $this->faker->bothify('SiteRedshopbOrder10structureCest product ?##?');
		...
		$this->company['name'] = $this->faker->bothify('SiteRedshopbOrder170structureCest company ?##?');
		$this->company['id'] = (int)$I->createCompany($this->company['name']);

		$I->comment('I prepare data to the delivery address');
		$this->deliveryAddress['name1'] = $this->faker->bothify('SiteRedshopbOrder170structureCest delivery_address ?##?');
		...
		$I->seeResponseContainsJson(
			[
				'_links' => [
					'curies' => [
						0 => [
							'href'      => "$baseUrl/index.php?option=com_redshopb&view=order&webserviceVersion=1.7.0&webserviceClient=site&format=doc&api=Hal#{rel}",
							'title'     => 'Documentation',
							'name'      => 'redshopb-order',
							'templated' => true
						]
					...
					'redshopb-order:list' => [
						'href'  => "$baseUrl/index.php?option=com_redshopb&view=order&webserviceVersion=1.7.0&webserviceClient=site&api=Hal"
					],
					'redshopb-order:self' => [
						'href'  => "$baseUrl/index.php?option=com_redshopb&view=order&webserviceVersion=1.7.0&webserviceClient=site&id=$this->order_id&api=Hal"
					],
				],
				'id'                                => $this->order_id,
				'delivery_address_id'               => $this->deliveryAddress['id'],
