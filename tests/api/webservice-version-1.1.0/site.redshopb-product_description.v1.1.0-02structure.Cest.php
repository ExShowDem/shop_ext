...
class SiteRedshopbProduct_description110structureCest
{
	/**
	 * @var
	 * @since 2.5.0
	...
	public function prepare(Step\Api\redshopb2b $I)
	{
		...
		$this->name  = $this->faker->bothify('SiteRedshopbProduct_description110structureCest product_description ?##?');
		$this->description = $this->faker->bothify('SiteRedshopbProductDescription110crudCest description ?##?');

		$this->product['name'] = (string) $this->faker->bothify('SiteRedshopbProduct_description110structureCest product ?##?');
		$this->product['sku']  = (int) $this->faker->randomNumber(3);
		$this->product['category_name'] = (string) $this->faker->bothify('SiteRedshopbProduct_description110structureCest category ?##?');
					...
					array (
						'curies' =>
							[
								0 =>
									[
										'href' => "$baseUrl/index.php?option=com_redshopb&view=product_description&webserviceVersion=1.1.0&webserviceClient=site&format=doc&api=Hal#{rel}",
										'title' => 'Documentation',
						...
						'redshopb-product_description:list' =>
							array (
								'href' => "$baseUrl/index.php?option=com_redshopb&view=product_description&webserviceVersion=1.1.0&webserviceClient=site&api=Hal",
							),
						'redshopb-product_description:self' =>
							array (
								'href' => "$baseUrl/index.php?option=com_redshopb&view=product_description&webserviceVersion=1.1.0&webserviceClient=site&id=$this->id&api=Hal",
							),
						'redshopb-product' =>
							array (
