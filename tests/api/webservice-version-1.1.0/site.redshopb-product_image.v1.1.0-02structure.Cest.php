...
/**
 * Class SiteRedshopbProductImage110StructureCest
 * @since 2.8.0
 */
class SiteRedshopbProductImage110StructureCest
{
	...
	public function prepare(redshopb2b $I)
	{
		...
		$this->productImage['name'] = $this->faker->bothify('SiteRedshopbProductImage110StructureCest product image ?##?');
		$this->productImage['erp_id'] = $this->faker->bothify('?##?');

		$I->comment('I create a category');
		$this->category['name'] = $this->faker->bothify('SiteRedshopbProductImage110StructureCest category ?##?');
		$this->category['id'] = (int) $I->createCategory($this->category['name'], '1.6.0');

		$I->comment('I create a product');
		$this->product['name'] = $this->faker->bothify('SiteRedshopbProductImage110StructureCest product ?##?');
					...
					[
						'curies' =>
							[
								0 =>
									[
										'href'      => "$baseUrl/index.php?option=com_redshopb&view=product_image&webserviceVersion=1.1.0&webserviceClient=site&format=doc&api=Hal#{rel}",
										'title'     => 'Documentation',
						...
						'redshopb-product_image:list' =>
							[
								'href' => "$baseUrl/index.php?option=com_redshopb&view=product_image&webserviceVersion=1.1.0&webserviceClient=site&api=Hal",
							],
						'redshopb-product_image:self' =>
							[
								'href' => "$baseUrl/index.php?option=com_redshopb&view=product_image&webserviceVersion=1.1.0&webserviceClient=site&id=" . $this->productImage['id'] . "&api=Hal",
							],
						'redshopb-product' =>
							[
