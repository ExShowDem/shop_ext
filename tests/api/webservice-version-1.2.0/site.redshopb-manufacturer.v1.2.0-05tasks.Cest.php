<?php
/**
 * @package     Aesir.E-Commerce
 * @subpackage  Tests
 *
 * @copyright   Copyright (C) 2012 - 2019 Aesir.E-Commerce All rights reserved.
 * @license     GNU General Public License version 2 or later, see LICENSE.
 */

class SiteRedshopbManufacturer120TasksCest
{
	/**
	 * @param ApiTester $I
	 */
	public function prepare(ApiTester $I)
	{
		$this->faker = Faker\Factory::create();
	}

	/**
	 * @param \Step\Api\redshopb2b $I
	 */
	public function taskPublish(Step\Api\redshopb2b $I)
	{
		$I->wantTo('PUBLISH a Manufacturer using GET');
		$I->comment('I create a manufacturer unpublished to be published');
		$manufacturerA['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer120tasksCest manufacturer ?##?');
		$manufacturerA['id'] = (int) $I->createManufacturer($manufacturerA['name'], '1.2.0');
		$I->unpublishManufacturer($manufacturerA['id'], '1.2.0');

		$I->wantTo('PUBLISH a Manufacturer using GET');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&task=publish'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. '&id=' . $manufacturerA['id']
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. '&id=' . $manufacturerA['id']
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['state' => true]);
		$I->deleteManufacturer($manufacturerA['id'], '1.2.0');
	}

	/**
	 * @param \Step\Api\redshopb2b $I
	 */
	public function taskUnpublish(Step\Api\redshopb2b $I)
	{
		$I->wantTo('UNPUBLISH a Manufacturer using GET');
		$I->comment('I create a manufacturer published to be unpublished');
		$manufacturerB['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer120tasksCest manufacturer ?##?');
		$manufacturerB['id'] = (int) $I->createManufacturer($manufacturerB['name'], '1.2.0');
		$I->publishManufacturer($manufacturerB['id'], '1.2.0');

		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&task=unpublish'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. '&id=' . $manufacturerB['id']
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. '&id=' . $manufacturerB['id']
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['state' => false]);

		$I->deleteManufacturer($manufacturerB['id'], '1.2.0');
	}

	/**
	 * @param \Step\Api\redshopb2b $I
	 */
	public function feature(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Feature a Manufacturer using GET');
		$I->comment('I create a manufacturer unfeatured to be featured');
		$manufacturerC['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer120TasksCest manufacturer ?##?');
		$manufacturerC['id'] = (int) $I->createManufacturer($manufacturerC['name'], '1.2.0');
		$I->unfeatureManufacturer($manufacturerC['id'], '1.2.0');

		$I->wantTo('Set a manufacturer as Featured');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. "&id=" . $manufacturerC['id']
			. '&task=feature'
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. "&id=" . $manufacturerC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['featured' => true]);
		$I->deleteManufacturer($manufacturerC['id'], '1.2.0');
	}

	/**
	 * @param \Step\Api\redshopb2b $I
	 */
	public function unfeature(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Set a manufacturer as UnFeatured');
		$manufacturerD['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer120TasksCest manufacturer ?##?');
		$manufacturerD['id'] = (int) $I->createManufacturer($manufacturerD['name'], '1.2.0');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			 . '?option=redshopb&view=manufacturer'
			 . '&api=Hal'
			 . '&webserviceClient=site'
			 . '&webserviceVersion=1.2.0'
			 . "&id=" . $manufacturerD['id']
			 . '&task=unfeature'
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. "&id=" . $manufacturerD['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['featured' => false]);
		$I->deleteManufacturer($manufacturerD['id'], '1.2.0');
	}

	/**
	 * @param \Step\Api\redshopb2b $I
	 */
	public function imageUploadAndImageRemove(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Upload an image to a Manufacturer using POST');
		$I->comment('I create a manufacturer');
		$manufacturerE['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer120TasksCest manufacturer ?##?');
		$manufacturerE['id'] = (int) $I->createManufacturer($manufacturerE['name'], '1.2.0');

		/**
		 * The following base64 code is a representation of a green 16px x 16px image
		 */
		$manufacturerE['image'] = "";

		$I->wantTo('Upload a Base64 encoded Image');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. '&task=imageUpload',
			[
				'id' => $manufacturerE['id'],
				'image' => $manufacturerE['image']
			]
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. "&id=" . $manufacturerE['id']
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->comment('I assert that image has been created');
		$I->dontSeeResponseContainsJson(['image' => null]);

		$image = $I->grabDataFromResponseByJsonPath('$.image');
		$I->assertRegExp("/(.*)media\/com_redshopb\/images\/originals\/manufacturers\/(.*).png/", $image[0], 'PNG Image has been created');

		$I->comment('I test image removal');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. '&task=imageRemove'
			. "&id=" . $manufacturerE['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.2.0'
			. "&id=" . $manufacturerE['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->comment('I assert that image has been removed');
		$I->seeResponseContainsJson(['image' => null]);

		$I->deleteManufacturer($manufacturerE['id'], '1.2.0');
	}
}
