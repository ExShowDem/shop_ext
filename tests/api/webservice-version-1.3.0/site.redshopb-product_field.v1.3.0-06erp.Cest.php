...
/**
 * Class SiteRedshopbProductField130erpCest
 * @since 2.6.1
 */
class SiteRedshopbProductField130erpCest
{
	...
	{
		$I->wantTo('prepare for POST a new product_field with erp id');
		$this->faker = Faker\Factory::create();

		$this->erpId = (int) $this->faker->numberBetween(100, 1000);
		$this->product_field['name'] = $this->faker->bothify('SiteRedshopbProduct_Field130erpCest field ?##?');
		$this->product_field['title'] = $this->faker->bothify('SiteRedshopbProduct_Field130erpCest field ?##?');
	...
	{
		$I->wantTo('POST a new product_field with erp id');
	...
	{
		$I->wantTo('GET an existing product_field with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=product_field'
			. '&api=Hal'
	...
	{
		$I->wantTo('UPDATE a product_field using PUT with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
	...
	{
		$I->wantTo('unpublish a product_field using GET with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=product_field'
			. '&task=unpublish'
	...
	{
		$I->wantTo('publish a product_field using GET with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=product_field'
			. '&task=publish'
	...
	{
		$I->wantTo('Create a translation for the product_field using POST with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
	...
	{
		$I->wantTo('Delete a translation for the product_field using POST with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=product_field'
			. '&api=hal'
	...
	{
		$I->wantTo('DELETE a product_field using DELETE with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
