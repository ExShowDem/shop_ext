...
/**
 * Class SiteRedshopbProductImage130ErpCest
 * @since 2.8.0
 */
class SiteRedshopbProductImage130ErpCest
{
	...
	public function prepare(redshopb2b $I)
	{
		...
		$this->productImage['name'] = $this->faker->bothify('SiteRedshopbProductImage130ErpCest product image ?##?');
		$this->productImage['erp_id'] = $this->faker->bothify('?##?');

		$I->comment('I create a category');
		$this->category['name'] = $this->faker->bothify('SiteRedshopbProductImage130ErpCest category ?##?');
		$this->category['id'] = (int) $I->createCategory($this->category['name'], '1.6.0');

		$I->comment('I create a product');
		$this->product['name'] = $this->faker->bothify('SiteRedshopbProductImage130ErpCest product ?##?');
	...
	{
		$I->wantTo('GET an existing product Image with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=product_image'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.3.0'
			. "&id=erp." . $this->productImage['erp_id']
		);
	...
	{
		$I->wantTo('UPDATE a product image using PUT with erp id');
	...
	{
		$I->wantTo('unpublish a product image using GET with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendGET('index.php'
			. '?option=redshopb&view=product_image'
			. '&task=unpublish'
	...
	{
		$I->wantTo('DELETE a product image with erp id');
		$I->amHttpAuthenticated('admin', 'admin');
