<?php
/**
 * @package     Aesir.E-Commerce
 * @subpackage  Tests
 *
 * @copyright   Copyright (C) 2012 - 2019 Aesir. All rights reserved.
 * @license     GNU General Public License version 2 or later, see LICENSE.
 */

class SiteRedshopbTag110TasksCest
{
	public function _before()
	{
		$this->faker = Faker\Factory::create();
	}

	public function unpublish(Step\Api\redshopb2b $I)
	{
		$I->wantTo('UNPUBLISH a tag');
		$I->amHttpAuthenticated('admin', 'admin');

		$tag = new stdClass;
		$tag->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tag->id = $I->createTag($tag->name, '1.1.0');

		$I->wantTo('UNPUBLISH tag with POST using its internal id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&task=unpublish"
			. "&id=$tag->id"
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&id=' . $tag->id
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['id' =>  $tag->id]);
		$I->seeResponseContainsJson(['name' =>  $tag->name]);
		$I->seeResponseContainsJson(['state' => false]);
		$I->comment("The tag $tag->name has been unpublished");

		// Clean up
		$I->deleteTag($tag->id, '1.1.0');
	}

	public function unpublishUnpublishesChildTags(Step\Api\redshopb2b $I)
	{
		$I->wantTo('UNPUBLISH a tag with other tags under it and they should get unpublished too');
		$I->amHttpAuthenticated('admin', 'admin');

		$tagParent = new stdClass;
		$tagParent->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tagParent->id = $I->createTag($tagParent->name, '1.1.0');

		$tagChild = new stdClass;
		$tagChild->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tagChild->id = $I->createTag(
			$tagChild->name,
			'1.1.0',
			['parent_id' => $tagParent->id]
		);

		$I->wantTo('UNPUBLISH Parent Tag');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&task=unpublish"
			. "&id=$tagParent->id"
		);

		$I->seeResponseCodeIs(200);

		$I->comment('I get child tag to check if has been unpublished');
		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&id=' . $tagChild->id
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['id' =>  $tagChild->id]);
		$I->seeResponseContainsJson(['name' =>  $tagChild->name]);
		$I->seeResponseContainsJson(['state' => false]);

		// Clean up
		$I->deleteTag($tagParent->id, '1.1.0');
	}

	public function publish(Step\Api\redshopb2b $I)
	{
		$I->wantTo('PUBLISH a tag');
		$I->amHttpAuthenticated('admin', 'admin');

		$tag = new stdClass;
		$tag->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tag->id = $I->createTag($tag->name, '1.1.0');
		$I->unpublishTag($tag->id, '1.1.0');

		$I->wantTo('UNPUBLISH tag with POST using its internal id');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&task=publish"
			. "&id=$tag->id"
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&id=' . $tag->id
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['id' => $tag->id]);
		$I->seeResponseContainsJson(['name' => $tag->name]);
		$I->seeResponseContainsJson(['state' => true]);
		$I->comment("The tag $tag->name has been published");

		// Clean up
		$I->deleteTag($tag->id, '1.1.0');
	}

	public function publishPublishesChildTags(Step\Api\redshopb2b $I)
	{
		$I->wantTo('UNPUBLISH a tag with other tags under it and they should get unpublished too');
		$I->amHttpAuthenticated('admin', 'admin');

		$tagParent = new stdClass;
		$tagParent->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tagParent->id = $I->createTag($tagParent->name, '1.1.0');

		$tagChild = new stdClass;
		$tagChild->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tagChild->id = $I->createTag(
			$tagChild->name,
			'1.1.0',
			['parent_id' => $tagParent->id]
		);
		$I->unpublishTag($tagChild->id, '1.1.0');

		$I->wantTo('PUBLISH Parent Tag');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&task=publish"
			. "&id=$tagParent->id"
		);

		$I->seeResponseCodeIs(200);

		$I->comment('I get child tag to check if has been published');
		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&id=' . $tagChild->id
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['id' => $tagChild->id]);
		$I->seeResponseContainsJson(['name' => $tagChild->name]);
		$I->seeResponseContainsJson(['state' => true]);

		// Clean up
		$I->deleteTag($tagParent->id, '1.1.0');
	}

	public function translate(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Create a translation for a Tag with POST using its internal id');
		$I->amHttpAuthenticated('admin', 'admin');
		$tag = new stdClass;
		$tag->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tag->id = $I->createTag($tag->name, '1.1.0');
		$tag->translatedName = 'french-' . $tag->name;
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&task=translate'
			. '&language=fr-FR'
			. "&id=$tag->id"
			. "&name=$tag->translatedName"
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=$tag->id"
			. '&language=fr-FR'
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['name' => $tag->translatedName]);

		// Clean up
		$I->deleteTag($tag->id, '1.1.0');
	}

	public function translateRemove(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Delete a translation for a Tag with POST using its internal id');
		$I->amHttpAuthenticated('admin', 'admin');
		$tag = new stdClass;
		$tag->name = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag ?##?');
		$tag->id = $I->createTag($tag->name, '1.1.0');
		$tag->translatedName = 'french-' . $tag->name;

		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&task=translate'
			. '&language=fr-FR'
			. "&id=$tag->id"
			. "&name=$tag->translatedName"
		);

		$I->seeResponseCodeIs(200);

		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&task=translateRemove'
			. '&language=fr-FR'
			. "&id=$tag->id"
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=$tag->id"
			. '&language=fr-FR'
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->dontSeeResponseContainsJson(['name' => $tag->translatedName]);
		$I->SeeResponseContainsJson(['name' => $tag->name]);

		// Clean up
		$I->deleteTag($tag->id, '1.1.0');
	}

	public function imageUploadAndImageRemove(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Upload an image to a Tag using POST');
		$I->comment('I create a tag');
		$tagC['name']  = (string) $this->faker->bothify('SiteRedshopbTag110TasksCest tag with image ?##?');
		$tagC['id'] = (int) $I->createTag($tagC['name'], '1.1.0');

		// The following base64 code is a representation of a green 16px x 16px image
		$tagC['image'] = "";


		$I->wantTo('Upload a Base64 encoded Image');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&task=imageUpload',
			[
				 'id' => $tagC['id'],
				 'image' => $tagC['image']
			]
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=" . $tagC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->comment('I assert that image has been created');
		$I->dontSeeResponseContainsJson(['image' => null]);

		$image = $I->grabDataFromResponseByJsonPath('$.image');
		$I->assertRegExp("/(.*)media\/com_redshopb\/images\/originals\/tags\/(.*).png/", $image[0], 'PNG Image has been created');

		$I->comment('I test image removal');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&task=imageRemove'
			. "&id=" . $tagC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->sendGET('index.php'
			. '?option=redshopb&view=tag'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=" . $tagC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->comment('I assert that image has been removed');
		$I->seeResponseContainsJson(['image' => null]);

		$I->deleteTag($tagC['id'], '1.1.0');
	}
}
