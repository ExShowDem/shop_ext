...
class SiteRedshopbProduct100tasksCest
{
	...
	public function taskPublish(Step\Api\redshopb2b $I)
	{
		...
		$product->name  = (string) $this->faker->bothify('SiteRedshopbProduct100tasksCest product ?##?');
		$product->sku  = $this->faker->randomNumber(3);
		$product->category_name = (string) $this->faker->bothify('SiteRedshopbProduct100FilteringCest category ?##?');
		...
		$I->unpublishProduct($product->id, '1.0.0');
		...
		// Clean up
		$I->deleteProduct($product->id, '1.0.0');
	}
	...
	public function taskTagAddandTagRemove(Step\Api\redshopb2b $I)
	{
		...
		$tag1 = new stdClass;
		$tag1->name = $this->faker->bothify('SiteRedshopbProduct100tasksCest tag1 ?##?');
		$tag1->id  = (int) $I->createTag($tag1->name);

		$tag2 = new stdClass;
		$tag2->name = $this->faker->bothify('SiteRedshopbProduct100tasksCest tag2 ?##?');
		...
		$I->deleteTag($tag2->id);
		$I->deleteProduct($product->id, '1.0.0');
		$I->deleteCategory($product->category_id, '1.0.0');
	}
}
