<?php
/**
 * @package     Aesir.E-Commerce
 * @subpackage  Tests
 *
 * @copyright   Copyright (C) 2012 - 2019 Aesir. All rights reserved.
 * @license     GNU General Public License version 2 or later, see LICENSE.
 */

class SiteRedshopbManufacturer110TasksCest
{
	/**
	 * Set up
	 */
	public function __construct()
	{
		$this->faker = Faker\Factory::create();
	}

	public function feature(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Feature a Manufacturer using GET');
		$I->comment('I create a manufacturer unfeatured to be featured');
		$manufacturerA['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer110TasksCest manufacturer ?##?');
		$manufacturerA['id'] = (int) $I->createManufacturer($manufacturerA['name'], '1.1.0');
		$I->unfeatureManufacturer($manufacturerA['id'], '1.1.0');

		$I->wantTo('Set a manufacturer as Featured');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=" . $manufacturerA['id']
			. '&task=feature'
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=" . $manufacturerA['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['featured' => true]);
		$I->deleteManufacturer($manufacturerA['id'], '1.1.0');
	}

	public function unfeature(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Set a manufacturer as UnFeatured');
		$manufacturerB['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer110TasksCest manufacturer ?##?');
		$manufacturerB['id'] = (int) $I->createManufacturer($manufacturerB['name'], '1.1.0');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
			 . '?option=redshopb&view=manufacturer'
			 . '&api=Hal'
			 . '&webserviceClient=site'
			 . '&webserviceVersion=1.1.0'
			 . "&id=" . $manufacturerB['id']
			 . '&task=unfeature'
		);

		$I->seeResponseCodeIs(200);

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=" . $manufacturerB['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();
		$I->seeResponseContainsJson(['featured' => false]);
		$I->deleteManufacturer($manufacturerB['id'], '1.1.0');
	}

	public function imageUploadAndImageRemove(Step\Api\redshopb2b $I)
	{
		$I->wantTo('Upload an image to a Manufacturer using POST');
		$I->comment('I create a manufacturer');
		$manufacturerC['name']  = (string) $this->faker->bothify('SiteRedshopbManufacturer110TasksCest manufacturer ?##?');
		$manufacturerC['id'] = (int) $I->createManufacturer($manufacturerC['name'], '1.1.0');

		// The following base64 code is a representation of a green 16px x 16px image
		$manufacturerC['image'] = "";


		$I->wantTo('Upload a Base64 encoded Image');
		$I->amHttpAuthenticated('admin', 'admin');
		$I->sendPOST('index.php'
					 . '?option=redshopb&view=manufacturer'
					 . '&api=Hal'
					 . '&webserviceClient=site'
					 . '&webserviceVersion=1.1.0'
					 . '&task=imageUpload',
					 [
						 'id' => $manufacturerC['id'],
						 'image' => $manufacturerC['image']
					 ]
		);

		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->sendGET('index.php'
					. '?option=redshopb&view=manufacturer'
					. '&api=Hal'
					. '&webserviceClient=site'
					. '&webserviceVersion=1.1.0'
					. "&id=" . $manufacturerC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->comment('I assert that image has been created');
		$I->dontSeeResponseContainsJson(['image' => null]);

		$image = $I->grabDataFromResponseByJsonPath('$.image');
		$I->assertRegExp("/(.*)media\/com_redshopb\/images\/originals\/manufacturers\/(.*).png/", $image[0], 'PNG Image has been created');

		$I->comment('I test image removal');
		$I->sendPOST('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. '&task=imageRemove'
			. "&id=" . $manufacturerC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->sendGET('index.php'
			. '?option=redshopb&view=manufacturer'
			. '&api=Hal'
			. '&webserviceClient=site'
			. '&webserviceVersion=1.1.0'
			. "&id=" . $manufacturerC['id']
		);
		$I->seeResponseCodeIs(200);
		$I->seeResponseIsJson();

		$I->comment('I assert that image has been removed');
		$I->seeResponseContainsJson(['image' => null]);

		$I->deleteManufacturer($manufacturerC['id'], '1.1.0');
	}
}
