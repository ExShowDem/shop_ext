...
class SiteRedshopbProduct100structureCest
{
	public function prepare(Step\Api\redshopb2b $I)
	{
		$I->wantTo('POST a new product');
		$this->faker = Faker\Factory::create();
		$this->name  = $this->faker->bothify('SiteRedshopbProduct100CrudCest product ?##?');
		$this->sku  = $this->faker->randomNumber(3);
		$this->category['name'] = $this->faker->bothify('SiteRedshopbProduct100CrudCest category ?##?');
				...
				[
					'curies' =>
						[
							0 =>
								[
									'href' => "$baseUrl/index.php?option=com_redshopb&view=product&webserviceVersion=1.0.0&webserviceClient=site&format=doc&api=Hal#{rel}",
									'title' => 'Documentation',
					...
					'redshopb-product:list' =>
							[
								'href' => "$baseUrl/index.php?option=com_redshopb&view=product&webserviceVersion=1.0.0&webserviceClient=site&api=Hal",
							],
					'redshopb-product:self' =>
							[
								'href' => "$baseUrl/index.php?option=com_redshopb&view=product&webserviceVersion=1.0.0&webserviceClient=site&id=$this->id&api=Hal",
							],
				],
			'id' => $this->id,
			'id_others' => [],
