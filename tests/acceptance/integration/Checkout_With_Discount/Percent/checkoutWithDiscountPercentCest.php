...
/**
 * Class checkoutWithDiscountPercentCest
 *
...
class checkoutWithDiscountPercentCest
{
	 ...
	 * @since 2.4.1
	 */
	public function checkoutDiscountPercent(OrderSteps $I, \Codeception\Scenario $scenario)
	{
		$I->doFrontEndLogin();
		$I->amGoingTo('Create all discount to be used by the Discount');
		$I = new AllDiscountsSteps($scenario);
		$I->createDiscountPercent($this->products['name'], $this->percentDiscount, $this->discountType, $this->salesType, $this->status, $this->currency);
		$I->doFrontEndLogout();

		$I->wantTo('Checkout with discount percent');
