 ...
 * @subpackage  system_redshopblayout
 *
...
/**
 * PlgSystemRedshopblayout installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.6.55
 */
class PlgSystemRedshopblayoutInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('redshopblayout'))
			)
				->loadResult();
