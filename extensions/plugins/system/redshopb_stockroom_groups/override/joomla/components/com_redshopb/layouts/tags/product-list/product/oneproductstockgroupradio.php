...
if ($isShop && $isOneProduct && $extThis->placeOrderPermission)
{
	...
	// Stockroom amount
	$stockrooms = PlgSystemRedshopb_Stockroom_Groups::getProductsStockroomData($productId);
	...
	foreach ($stockrooms as $stockroom)
	{
		...
		$name = 'stockroomGroupId_' . $productId . $cartPrefix;
				...
				case 'actual_stock':
					...
					else
					{
						$stockAmount = Text::_('COM_REDSHOPB_SHOP_IN_STOCK') . ': '
							. RedshopbHelperProduct::decimalFormat($stockroom->amount, $productId);
					}
				...
				case 'color_codes':
					switch ($stockPresented)
					{
						case 'semaphore':
							...
							{
								$colorAmount = RedshopbHelperProduct::getColorAmount($productData);
							}
