 ...
 * @subpackage  redshipping_default_shipping
 *
...
/**
 * PlgRedshippingDefault_Shipping installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.6.55
 */
class PlgRedshippingDefault_ShippingInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('default_shipping'))
			)
				->loadResult();
