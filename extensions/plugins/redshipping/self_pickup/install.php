 ...
 * @subpackage  redshipping_self_pickup
 *
...
/**
 * PlgRedshippingSelf_Pickup installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.6.55
 */
class PlgRedshippingSelf_PickupInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('self_pickup'))
			)
				->loadResult();
