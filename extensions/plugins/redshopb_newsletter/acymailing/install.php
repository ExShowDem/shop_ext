 ...
 * @subpackage  redshopb_newsletter_acymailing
 *
...
/**
 * PlgRedshopb_NewsletterAcymailing installer class.
 *
 * @package  Redshopb.Plugin
 * @since    1.6.55
 */
class PlgRedshopb_NewsletterAcymailingInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('folder') . ' = ' . $db->quote('redshopb_newsletter'))
					->where($db->qn('element') . ' = ' . $db->quote('acymailing'))
			)
				->loadResult();
