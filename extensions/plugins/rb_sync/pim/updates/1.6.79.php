...
class PimUpdateScript_1_6_79
{
	...
	public function execute()
	{
		...
		$query = $db->getQuery(true)
			->select('s.local_id, s.metadata, f.*')
			->from($db->qn('#__redshopb_sync', 's'))
			->leftJoin($db->qn('#__redshopb_field', 'f') . ' ON f.id = s.local_id')
			->where('s.reference = ' . $db->q('erp.pim.field'));
			...
			foreach ($results as $result)
			{
				...
				if ($metaData)
				{
					...
					$query->clear()
						...
						->where('reference = ' . $db->q('erp.pim.field'))
