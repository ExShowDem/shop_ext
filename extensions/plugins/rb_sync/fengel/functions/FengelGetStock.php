...
/**
 * Get Stock amount function.
 *
 * @package     Aesir.E-Commerce.Plugin
 * @subpackage  Fengel
 * @since       1.0
 */
class FengelGetStock extends FengelFunctionBase
{
	/**
	...
	public function read(&$webserviceData, $params)
	{
		...
		try
		{
			$xml = $this->client->Red_GetItemVariantStock();
			...
			foreach ($xml->ItemVariantStock as $obj)
			{
			...
			$query->clear()
				->select(array('s1.local_id', 's1.remote_key', $db->qn('s2.remote_key', 'remote_product_key'), 'amount'))
			...
			foreach ($xml->ItemVariantStock as $obj)
			{
				...
				foreach ($obj->Keys->Key as $key)
				{
					...
					// @ToDo write new stock update code here-issue RSBTB-1963
				}
			}
