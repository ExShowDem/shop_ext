...
/**
 * Discount Debtor Group Model
 *
 * @package     Aesir.E-Commerce.Backend
 * @subpackage  Models
 * @since       1.0
 */
class RedshopbModelDiscount_Debtor_Group extends RedshopbModelAdmin
{
	/**
	 ...
	 * @return  Table
	 */
	public function getTable($name = 'customer_discount_group', $prefix = '', $config = array())
	{
		if ($name == '')
		{
		...
		if ($item->company_id != '')
		{
			if (!RedshopbHelperCompany::isChildOf($data['company_id'], $item->company_id))
			{
				Factory::getApplication()->enqueueMessage(
					Text::sprintf('COM_REDSHOPB_COMPANY_NOT_CHILDREN_OF', $data["company_id"], $item->id), 'error'
				);
