	...
	<?php else : ?>
		...
		<?php foreach ($preparedItems->items as $item): ?>
			<div class="productList container-fluid">
				<div class="row">
					<div class="col-md-12">
						<?php if ($item->date_new != '0000-00-00' && strtotime($item->date_new) && time() <= strtotime('+' . $config->getInt('date_new_product', 14) . ' day', strtotime($item->date_new))): ?>
							<span class="badge"><?php echo Text::_('COM_REDSHOPB_SHOP_NEW'); ?></span>&nbsp;
						<?php endif; ?>
				...
				<div class="row">
					<div class="col-md-2 productThumbs" id="productThumbs_<?php echo $collectionId; ?>_<?php echo $item->id; ?>">
							...
							<ul class="slides">
								<?php if (isset($preparedItems->productImages[$item->id])): ?>
									<?php foreach ($preparedItems->productImages[$item->id] as $image):
										?>
										<li>
												<a href="<?php echo RedshopbRoute::_('index.php?option=com_redshopb&task=shop.ajaxGetZoomImage&productId=' . $item->id . (isset($preparedItems->dropDownSelected[$item->id]) ? '&flatAttrId=' . $preparedItems->dropDownSelected[$item->id] : '') . '&mediaId=' . $image->id);?>" class="btn btn-link btn-sm pull-right" id="zoomLink<?php echo $item->id . '_' . $preparedItems->collectionId; ?>" data-toggle="modal" data-target="#zoomModal">
													<div class="thumbnail">
					...
					<div class="col-md-10">
						<div class="row">
								...
								<div class="col-md-3" id="accessory_<?php echo $collectionId; ?>_<?php echo $item->id; ?>">
									<?php
								...
								<div class="col-md-3">
									...
									?>
									<button class="btn btn-info btn-sm add-to-cart" type="button" onclick="redSHOPB.shop.addToCart(event);" name="addtocart_<?php echo $item->id; ?>_<?php echo $collectionId; ?>">
									<i class="icon-shopping-cart"></i>
									<?php echo Text::_('COM_REDSHOPB_SHOP_ADD_TO_CART'); ?>
									</button>
									<button class="btn btn-sm clearAmounts" type="button" id="clearAmounts_<?php echo $collectionId; ?>_<?php echo $item->id; ?>"
						...
						<div class="row" id="tableVariants_<?php echo $collectionId; ?>_<?php echo $item->id; ?>">
							<?php
