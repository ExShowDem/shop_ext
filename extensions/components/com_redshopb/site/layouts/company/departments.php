...
<div class="redshopb-company-departments">
	<form action="<?php echo $action; ?>" name="<?php echo $formName; ?>" class="adminForm" id="<?php echo $formName; ?>" method="post">
		...
		<?php else : ?>
			<table class="table table-striped table-hover  footable js-redshopb-footable redshopb-footable toggle-circle-filled" id="departmentList">
				<thead>
				<tr>
					<th width="1%">
					...
					<th class="nowrap" data-toggle="true">
						<?php echo HTMLHelper::_('rsearchtools.sort', 'COM_REDSHOPB_NAME', 'd.name', $listDirn, $listOrder, null, 'asc', '', null, $formName); ?>
					</th>
					<th width="4%" class="nowrap" data-hide="phone">
						<?php echo Text::_('COM_REDSHOPB_USERS') ?>
					</th>
					<th width="18%" class="nowrap" data-hide="phone">
						<?php echo HTMLHelper::_('rsearchtools.sort', 'COM_REDSHOPB_ADDRESS_LABEL', 'd.address', $listDirn, $listOrder, null, 'asc', '', null, $formName); ?>
					</th>
					<th width="8%" class="nowrap" data-hide="phone">
						<?php echo HTMLHelper::_('rsearchtools.sort', 'COM_REDSHOPB_ZIP_LABEL', 'd.zip', $listDirn, $listOrder, null, 'asc', '', null, $formName); ?>
					</th>
					<th width="12%" class="nowrap" data-hide="phone">
						<?php echo HTMLHelper::_('rsearchtools.sort', 'COM_REDSHOPB_CITY_LABEL', 'd.city', $listDirn, $listOrder, null, 'asc', '', null, $formName); ?>
					</th>
					<th width="12%" class="nowrap" data-hide="phone">
						<?php echo HTMLHelper::_('rsearchtools.sort', 'COM_REDSHOPB_COUNTRY_LABEL', 'd.country', $listDirn, $listOrder, null, 'asc', '', null, $formName); ?>
					</th>
					<th width="1%" class="nowrap" data-hide="phone">
						<?php echo HTMLHelper::_('rsearchtools.sort', 'JGRID_HEADING_ID', 'd.id', $listDirn, $listOrder, null, 'asc', '', null, $formName); ?>
					</th>
				</tr>
				</thead>
				<?php if ($items): ?>
							...
							<td>
								<?php if ($item->checked_out) : ?>
									<?php echo HTMLHelper::_('rgrid.checkedout', $i, $item->checked_out > 0 ? Factory::getUser($item->checked_out)->get('name') : '',
										$item->checked_out_time, 'departments.', $canCheckin, 'cb', $formName
									); ?>
								<?php endif; ?>
								<?php
								$itemUrl = 'index.php?option=com_redshopb&task=department.edit&id=' . $item->id
