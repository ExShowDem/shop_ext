...
if (RedshopbHelperACL::getPermission('manage', 'product', array('create'), true))
{
	$buttons[] = '<a class="btn btn-success"' . $disabled
		. ' href="javascript:void(0);" onclick="redSHOPB.form.submit(event);" data-task="product_composition.add">'
		. '<i class="' . $icons['new'] . '"></i> ' . Text::_('JTOOLBAR_NEW') . '</a>';
}

if (RedshopbHelperACL::getPermission('manage', 'product', array('edit'), true))
{
	$buttons[] = '<a class="btn btn-default"' . $disabled
		. ' href="javascript:void(0);" onclick="redSHOPB.form.submit(event);" data-task="product_composition.edit" data-list="true">'
...
if (RedshopbHelperACL::getPermission('manage', 'product', array('delete'), true))
{
	$buttons[] = '<a class="btn btn-danger"' . $disabled
		. ' href="javascript:void(0);" onclick="redSHOPB.products.tabSubmit(event);" data-task="product_compositions.delete" data-list="true">'
