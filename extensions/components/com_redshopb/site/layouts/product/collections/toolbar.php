...
if (RedshopbHelperACL::getPermission('manage', 'collection', array('create'), true))
{
	$buttons[] = '<a class="btn btn-success"'
		. ' href="javascript:void(0);" onclick="redSHOPB.form.submit(event);" data-task="collection.create">'
		. '<i class="' . $icons['new'] . '"></i> ' . Text::_('JTOOLBAR_NEW') . '</a>';
}

if (RedshopbHelperACL::getPermission('manage', 'collection', array('edit'), true))
{
	$buttons[] = '<a class="btn btn-default"'
		. ' href="javascript:void(0);" onclick="redSHOPB.form.submit(event);" data-task="collection.edit" data-list="true">'
...
if (RedshopbHelperACL::getPermission('manage', 'collection', array('delete'), true))
{
	$buttons[] = '<a class="btn btn-danger"'
		. ' href="javascript:void(0);" onclick="redSHOPB.products.tabSubmit(event);" data-task="collections.delete" data-list="true">'
