...
<script type="text/javascript">
	...
	(function ($) {
		$(document).ready(function () {
			<?php if (!$canReadOnly): ?>
			$('.product-complimentary-add').click(function(event){
				event.preventDefault();

				var complimentaryProductId = $(this).attr('data-complimentaryproductid');
				var productId = $(this).attr('data-productid');

				var url = '<?php echo Uri::root(); ?>index.php?option=com_redshopb&task=product.ajaxAddProductComplimentary';
				...
				.done(function(data){
					if (data == 1) {
						$('#complimentaryproductList').find('#complimentary-row-msg' + complimentaryProductId).show().html('Added Successfully');
						redSHOPB.products.loadTab('selectedcomplimentaryproducts');
					}
					else
					{
						$('#complimentaryproductList').find('#complimentary-row-msg' + complimentaryProductId).show().html('Complimentary Product Already Added');
					}
					setTimeout(function() {
						$('#complimentary-row-msg' + complimentaryProductId).fadeOut('fast');
					}, 1000);
				});
			});
			<?php endif; ?>
...
<?php echo RedshopbLayoutHelper::render('searchtools.default', $searchToolsOptions); ?>
	...
	<?php else : ?>
		<table class="table table-striped table-hover footable js-redshopb-footable redshopb-footable toggle-circle-filled" id="complimentaryproductList">
			<thead>
			<tr>
				<th width="1%">
			...
			<?php if ($items): ?>
				...
				<?php foreach ($items as $i => $item): ?>
					...
					<tr id="product-row-<?php echo $item->id; ?>">
						...
						<td>
						<div id ="complimentary-row-msg<?php echo $item->id; ?>" class="alert alert-info" style="display: none;" ></div>

						<?php if (!$canReadOnly): ?>
						<br/>
						<a class="product-item-add product-complimentary-add" href="javascript:void(0);"
								data-complimentaryproductid="<?php echo $item->id; ?>"
								data-productid="<?php echo $productId; ?>">
								<button><?php echo Text::_('COM_REDSHOPB_COMPLIMENTARY_ADD');?></button>
							</a>
						<?php endif ?>
						</td>
						<td>
