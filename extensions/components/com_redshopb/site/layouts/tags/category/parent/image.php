...
if ($category->isLoaded() && (int) $category->get('parent_id') > 1):
	...
	if (!empty($parent->get('image'))):
		$imagePath = RedshopbHelperThumbnail::getFullImagePath($parent->get('image'), 'categories');

		if ($imagePath):
			?>
			<img src="<?php echo Uri::root() . $imagePath ?>"
				 alt="<?php echo RedshopbHelperThumbnail::safeAlt($parent->get('name')) ?>"/>
			<?php
		endif;
	else:
