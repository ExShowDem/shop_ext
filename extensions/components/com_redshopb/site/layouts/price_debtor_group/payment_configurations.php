...
$paymentConfigurationId = $data['paymentConfigurationId'];
$state                  = $data['state'];
$items                  = $data['items'];
...
<div class="redshopb-price-debtor-group-payment-configurations">
		<?php
		// Render the toolbar?
		if ($showToolbar)
		{
			echo RedshopbLayoutHelper::render('price_debtor_group.payment_configurations_toolbar', $data);
		}
		?>
	<form action="<?php echo $action; ?>" name="<?php echo $formName; ?>" class="adminForm" id="<?php echo $formName; ?>" method="post">
		...
		<?php else : ?>
			<table class="table table-striped table-hover footable js-redshopb-footable redshopb-footable toggle-circle-filled" id="paymentConfigurationList">
				<thead>
				<tr>
					<th width="1%">
				...
				<?php if ($items): ?>
					...
					<?php foreach ($items as $i => $item): ?>
						...
						$name   = $params->get('payment_title', $item->plugin_name);
						?>
						<tr>
							<td>
							...
							<td>
								<?php echo HTMLHelper::_('rgrid.published', $item->state, $i, 'payment_configurations.', $canChange, 'cb', null, null, $formName); ?>
							</td>
							<td>
								<?php if ($item->checked_out) : ?>
									<?php echo HTMLHelper::_('rgrid.checkedout', $i, $item->checked_out > 0 ? Factory::getUser($item->checked_out)->get('name') : '',
										$item->checked_out_time, 'payment_configurations.', $canCheckin, 'cb', $formName
									); ?>
								<?php endif; ?>
								<?php
								$itemUrl = 'index.php?option=com_redshopb&task=payment_configuration.edit&id=' . $item->id
									. '&jform[price_debtor_group_id]=' . $paymentConfigurationId . '&from_price_debtor_group=1';
		...
		<div>
			...
			<?php endif; ?>
			<input type="hidden" name="filter[price_debtor_group]" value="<?php echo $paymentConfigurationId ?>">
