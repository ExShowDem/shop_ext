 ...
 * @since       1.6
 */
class Com_RedshopbUpdateScript_1_6_7
{
	/**
	...
	public function execute()
	{
		...
		$this->fillTableAliases('#__redshopb_conversion', 'Conversion');
		$this->fillTableAliases('#__redshopb_company', 'Company', array('name'), array('parent_id'));
		$this->fillTableAliases('#__redshopb_category', 'Category', array('name'), array('parent_id'));
		$this->fillTableAliases('#__redshopb_department', 'Department', array('name'), array('parent_id'));
		$this->fillTableAliases('#__redshopb_cron', 'Cron', array('plugin', 'name'));
		$this->fillTableAliases('#__redshopb_product', 'Product', array('name'), array('company_id', 'category_id'));
		$this->fillTableAliases('#__redshopb_tag', 'Tag', array('type', 'name'), array('parent_id'));
		$this->fillTableAliases('#__redshopb_type', 'Type');
		$this->fillTableAliases('#__redshopb_product_attribute', 'Product_Attribute', array('name'), array('product_id'));
		$this->fillTableAliases('#__redshopb_stockroom', 'Stockroom', array('name'), array('company_id'));
		$this->fillTableAliases('#__redshopb_field', 'Field');

		$db    = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->update(array($db->qn('#__redshopb_cron', 'c'), $db->qn('#__redshopb_cron', 'cp')))
			->set($db->qn('c.parent_alias') . ' = ' . $db->qn('cp.alias'))
			->where($db->qn('c.parent_id') . ' = ' . $db->qn('cp.id'))
