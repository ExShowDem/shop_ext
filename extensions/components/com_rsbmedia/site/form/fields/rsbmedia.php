 ...
 * @since       1.6
 */
class JFormFieldRsbmedia extends FormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		if (!self::$initialised)
		{
			...
			$script[] = '			if (value) {';
			$script[] = '				$img.attr("src", "' . Uri::root() . '" + value);';
		...
		if ($showPreview)
		{
			...
			if ($showAsTooltip)
			{
				...
				$tooltip = '<div class="well" style="z-index:100000">' . $previewImgEmpty . $previewImg . '</div>';
				$options = array(
		...
		if ($this->disabled != true)
		{
			...
			$html[] = '<a class="modal btn" title="' . Text::_('JLIB_FORM_BUTTON_SELECT') . '" href="'
				. ($this->readonly ? ''
					: ($this->link ? $this->link
						: 'index.php?option=com_rsbmedia&amp;view=images&amp;tmpl=component&amp;asset=' . $asset . '&amp;author='
						. $this->form->getValue($this->authorField)) . '&amp;fieldid=' . $this->id) . '"'
				. ' rel="{handler: \'iframe\', size: {x: 800, y: 500}}">';
			$html[] = Text::_('JLIB_FORM_BUTTON_SELECT') . '</a><a class="btn hasTooltip" title="' . Text::_('JLIB_FORM_BUTTON_CLEAR')
				. '" href="#" onclick="';
