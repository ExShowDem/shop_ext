...
/**
 * Installer script for RedshopB2B Manufacturers
 *
 * @package     Aesir.E-Commerce.Modules
 * @subpackage  Modules
 * @since       1.6.55
 */
class Mod_Redshopb_ManufacturersInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('mod_redshopb_manufacturers'))
			)
				->loadResult();
			...
			if (!empty($version))
			{
				...
				if (version_compare($version, '1.6.34', '<'))
				{
