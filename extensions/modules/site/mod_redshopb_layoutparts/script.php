 ...
 * @subpackage  mod_redshopb_layoutparts
 *
...
/**
 * Installer script for RedshopB2B LayoutParts
 *
 * @package     Aesir.E-Commerce.Modules
 * @subpackage  Modules
 * @since       1.6.55
 */
class Mod_Redshopb_LayoutpartsInstallerScript
{
	/**
		...
		if ($type == 'update' || $type == 'discover_install')
		{
			...
			$version = $db->setQuery(
				$db->getQuery(true)
					...
					->where($db->qn('element') . ' = ' . $db->quote('mod_redshopb_layoutparts'))
			)
				->loadResult();
